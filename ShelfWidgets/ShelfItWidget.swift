...
/// Small Home Screen widget: streak + a one-tap "Shelf it" button. The button runs the same
/// intent Siri and the Action Button use, which opens the app and starts a session.
struct ShelfItWidget: Widget {
    var body: some WidgetConfiguration {
        StaticConfiguration(kind: "ShelfItWidget", provider: StreakProvider()) { entry in
            ShelfItWidgetView(entry: entry)
                .containerBackground(Color(red: 0.07, green: 0.07, blue: 0.09), for: .widget)
        }
        .configurationDisplayName("Shelf it")
...
private struct ShelfItWidgetView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Spacer()
            if entry.isActive {
                Label("On the shelf", systemImage: "books.vertical.fill")
                    .font(.system(.subheadline, design: .rounded).weight(.semibold))
                    .foregroundStyle(.white)
            } else {
                Button(intent: StartShelfSessionIntent()) {
                    Text("Shelf it")
