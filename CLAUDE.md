# Shelf — notes for Claude Code sessions

iOS app (SwiftUI, iOS 17+). Read `README.md` and `docs/01-strategy.md` first.

## Build
- The Xcode project is **generated**: `xcodegen generate` (brew install xcodegen). Never commit `Shelf.xcodeproj`.
- Any change to targets, files outside existing folders, entitlements or Info.plist keys goes in `project.yml`, then regenerate.
- Build: `xcodebuild build -project Shelf.xcodeproj -scheme Shelf -destination 'platform=iOS Simulator,name=<an iPhone>' CODE_SIGNING_ALLOWED=NO`
- Tests: same with `test -only-testing:ShelfTests`.
...
- Dark theme only, tokens in `Shelf/Design/Theme.swift`.
