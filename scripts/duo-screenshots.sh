#!/bin/bash
# Build Clam for the iPhone Duo simulator, run the unit tests there, and capture every screen.
#
...
if [ "$RUN_TESTS" = 1 ]; then
  xcodebuild test -project Clam.xcodeproj -scheme Clam -destination "$DEST" -derivedDataPath DerivedData \
    -only-testing:ClamTests -skipPackagePluginValidation CODE_SIGNING_ALLOWED=NO 2>&1 | tee duo-test.log \
