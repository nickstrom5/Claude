#!/usr/bin/env bash
# One-shot Cloudflare setup for getclam.app: DNS for GitHub Pages + email forwarding.
...
#              Zone:DNS:Edit, Zone:Zone:Read, scoped to getclam.app.
set -euo pipefail

DOMAIN="${DOMAIN:-getclam.app}"
FORWARD_TO="${FORWARD_TO:-Nickstrom5@gmail.com}"
GITHUB_USER="${GITHUB_USER:-nickstrom5}"
GITHUB_TXT_VALUE="${GITHUB_TXT_VALUE:-f608cf247e8b8dbac47ce879e36ee9}"
...
echo "  4. Delete the clam-setup API token in Cloudflare."
