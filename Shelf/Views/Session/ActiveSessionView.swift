    ...
    var body: some View {
        if let session = sessions.active {
            VStack(spacing: 0) {
                ...
                Text("\(screenTime.selectedCount) apps on the shelf")
...
struct SessionResultView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            Text(session.completed ? "Shelved for \(session.formattedDuration)." : "Stopped at \(session.formattedDuration).")
                .font(Theme.Font.title)
                .foregroundStyle(Theme.textPrimary)
                .padding(.top, 18)
            Text(session.completed
                 ? "\(appState.stats.streak)-day streak. \(appState.stats.totalFormatted) shelved total."
            ...
            if session.completed {
                ShareCardView(title: "I shelved my phone", detail: "for \(session.formattedDuration)", streak: appState.stats.streak)
                    .padding(.top, 28)
                    .scaleEffect(0.9)
            }
            Spacer()
        ...
        .sheet(isPresented: $showShare) {
            ShareSheet(items: [ShareCardView(title: "I shelved my phone", detail: "for \(session.formattedDuration)", streak: appState.stats.streak).render()].compactMap { $0 })
        }
    }
}
