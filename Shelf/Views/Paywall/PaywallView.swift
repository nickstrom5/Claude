...
struct PaywallView: View {
    ...
    private var header: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text("Try Shelf free for 7 days")
    ...
    private func scheduleTrialReminder() {
        ...
        center.requestAuthorization(options: [.alert, .sound]) { granted, _ in
            ...
            content.title = "Your Shelf trial ends in 2 days"
            ...
            center.add(UNNotificationRequest(identifier: "shelf.trialReminder", content: content, trigger: trigger))
        }
    }
}
        ...
        Button(action: onTap) {
            HStack(spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 3) {
                    HStack(spacing: 8) {
                        Text(product.displayName.replacingOccurrences(of: "Shelf ", with: ""))
                            .font(Theme.Font.headline)
                            .foregroundStyle(Theme.textPrimary)
                        if let badge {
                            Text(badge)
